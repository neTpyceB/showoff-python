...
import showoff_event.feed_app as feed_app_module
...
from showoff_event.store import FeedStore
...
def test_feed_app_reads_projected_entries_and_manages_consumer(tmp_path) -> None:
    ...
    store = FeedStore(settings.feed_db_path)
    app = create_feed_app(settings=settings, store=store, consumer=consumer)

    with TestClient(app) as client:
        store.add_event(make_event())
        response = client.get("/users/alice/feed")
        health = client.get("/health")

    assert response.status_code == 200
    assert response.json()[0]["title"] == "Ride completed"
    assert health.json() == {"status": "ok"}
    assert state == {"started": True, "stopped": True}


def test_feed_app_default_consumer_projects_event(tmp_path, monkeypatch) -> None:
    settings = make_settings(tmp_path)
    store = FeedStore(settings.feed_db_path)
    ...
    monkeypatch.setattr(feed_app_module, "consume_events", fake_consume_events)

    with TestClient(create_feed_app(settings=settings, store=store)) as client:
        response = client.get("/users/alice/feed")
