...
import showoff_event.audit_app as audit_app_module
...
from showoff_event.store import AuditStore
...
def test_audit_app_reads_projected_entries_and_manages_consumer(tmp_path) -> None:
    ...
    app = create_audit_app(settings=settings, store=store, consumer=consumer)
    ...
    assert response.json()[0]["type"] == "activity.created"
    assert health.json() == {"status": "ok"}
    assert state == {"started": True, "stopped": True}


def test_audit_app_default_consumer_projects_event(tmp_path, monkeypatch) -> None:
    settings = make_settings(tmp_path)
    store = AuditStore(settings.audit_db_path)
    ...
    monkeypatch.setattr(audit_app_module, "consume_events", fake_consume_events)

    with TestClient(create_audit_app(settings=settings, store=store)) as client:
        response = client.get("/events")
